...
export const getEspecialidadById = async (id) => {
  const [rows] = await pool.query("SELECT * FROM especialidades WHERE id_especialidad = ?", [id]);
  return rows[0];
};
...
export const deleteEspecialidad = async (id) => {
  const [result] = await pool.query("DELETE FROM especialidades WHERE id_especialidad = ?", [id]);
  return result;
};
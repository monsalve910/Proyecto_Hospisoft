...
export const getMedicos = async () => {
  const [rows] = await pool.query("SELECT m.id_medico,m.nombre,m.apellido,m.telefono,m.email,m.especialidad_id,e.nombre_especialidad AS especialidad FROM medico m INNER JOIN especialidades e ON m.especialidad_id = e.id_especialidad");
  return rows;
};

//Obtener un medico por ID
export const getMedicoById = async (id) => {
  const [rows] = await pool.query("SELECT m.id_medico,m.nombre,m.apellido,m.telefono,m.email,m.especialidad_id,e.nombre_especialidad AS especialidad FROM medico m INNER JOIN especialidades e ON m.especialidad_id = e.id_especialidad WHERE m.id_medico = ?", [id]);
  return rows[0];
};
